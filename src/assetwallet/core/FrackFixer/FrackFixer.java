...
public class FrackFixer extends Servant {
    ...
    public void doMove(ArrayList<Asset> ccs) {
        int cnt = 0;
        ...
        for (Asset cc : ccs) {
            ...
            for (int i = RAIDA.TOTAL_RAIDA_COUNT - 1; i >= 0; i--) {
                if (cc.getDetectStatus(i) != Asset.STATUS_FAIL)
                    cnt++;
            }
    ...
    public void doFrackFix() {
        ...
        Asset cc;
        ArrayList<Asset> ccall = new ArrayList<Asset>();
        ...
        ArrayList<Asset> ccactive = new ArrayList<Asset>();
        int corner, i;
        ...
        for (i = 0; i < RAIDA.TOTAL_RAIDA_COUNT; i++) {
            ...
            for (Asset tcc : ccall) {
                if (tcc.getDetectStatus(i) == Asset.STATUS_PASS)
                    continue;
    ...
    private void doRealFix(int raidaIdx, ArrayList<Asset> ccs) {
        int corner;
    ...
    public boolean fixCoinsInCorner(int raidaIdx, int corner, ArrayList<Asset> ccs) {
        int[] triad;
        int[] raidaFix;
        int neighIdx;
        boolean first = true;
    ...
    private void syncCoins(int raidaIdx, ArrayList<Asset> ccs) {
        for (Asset cc : ccs)
            syncCoin(raidaIdx, cc);
    }
    
    private void syncCoin(int raidaIdx, Asset cc) {
        logger.info(ltag, "Syncing " + cc.originalFile);

        cc.setDetectStatus(raidaIdx, Asset.STATUS_PASS);
